...
Spike <- 200 # Delta_dist threshhold for declaring a spike (feet)

for (i in 1:nrow(df_utm)) {
  if (df_utm[i,]$Delta_dist>Spike) {
...
for (i in 1:nrow(df_utm3)) {
  if (df_utm3[i,]$Delta_dist>Spike) {
    # NewX <- (df_utm2[i-1,]$X + df_utm2[i+1,]$X)/2
    # NewY <- (df_utm2[i-1,]$Y + df_utm2[i+1,]$Y)/2
    # new_point <- sf::st_point(c(NewX, NewY)) %>% 
...
# Spike <- 100 # Delta_dist threshhold for declaring a spike (feet)
# 
# df_utm3 <- df_utm2 %>% 
#   group_by(Date) %>% 
#     mutate(NewX=(lag(X, default=first(X)) + lead(X, default=last(X)))/2) %>% 
...
R<-function(k) diag(2)* 50 #estimate of measurement variance
# Q<-function(k) diag(2)* as.numeric(day$timeNextMeasure[k])^1.5# the process variance
Q<-function(k) diag(2)* 50 # the process variance
#initialise guesses:
xhat[1,] <- z[1,]
P[,,1] <- diag(2)
for (k in 2:count){
...
New <- as.data.frame(xhat) %>% 
  sf::st_as_sf(coords=c("V1", "V2"), crs=Seattle)
